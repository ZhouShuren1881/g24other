...
public class AuthFilter implements GatewayFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (StringUtil.isNullOrEmpty(token)){
            ...
            message.put("errno", 704);
            message.put("errmsg", "需要先登录");
        ...
        if (userAndDepart == null) {
            ...
            // 设置返回消息
            JSONObject message = new JSONObject();
            message.put("errno", 501);
            message.put("errmsg", "JWT不合法");
            byte[] bits = message.toJSONString().getBytes(StandardCharsets.UTF_8);
            DataBuffer buffer = response.bufferFactory().wrap(bits);
            //指定编码，否则在浏览器中会中文乱码
            response.getHeaders().add("Content-Type", "text/plain;charset=UTF-8");
            ...
            for (String singleBanSetName : banSetName) {
                ...
                if (redisTemplate.hasKey(singleBanSetName)) {
                    ...
                    if (redisTemplate.opsForSet().isMember(singleBanSetName, token)) {
                        // 设置返回消息
                        JSONObject message = new JSONObject();
                        message.put("errno", 705);
                        message.put("errmsg", "无权限访问");
                        byte[] bits = message.toJSONString().getBytes(StandardCharsets.UTF_8);
                        DataBuffer buffer = response.bufferFactory().wrap(bits);
                        //指定编码，否则在浏览器中会中文乱码
                        response.getHeaders().add("Content-Type", "text/plain;charset=UTF-8");
            ...
            if (url != null) {
                ...
                if (pathId != null && !departId.equals(0L)) {
                    ...
                    if (!pathId.equals(departId.toString())) {
                        ...
                        // 设置返回消息
                        JSONObject message = new JSONObject();
                        message.put("errno", 505);
                        message.put("errmsg", "操作的资源id不是自己的对象");
                        byte[] bits = message.toJSONString().getBytes(StandardCharsets.UTF_8);
                        DataBuffer buffer = response.bufferFactory().wrap(bits);
                        //指定编码，否则在浏览器中会中文乱码
                        response.getHeaders().add("Content-Type", "text/plain;charset=UTF-8");
